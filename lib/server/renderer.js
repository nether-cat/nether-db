...
let bundleRenderer = createBundleRenderer(serverBundle, options);
...
const reload = function (serverBundle, clientManifest) {
  bundleRenderer = createBundleRenderer(serverBundle, { ...options, clientManifest });
};
...
const needsRedirect = function (ctx, renderContext, exception) {
  let { needsRedirect: location } = exception;
